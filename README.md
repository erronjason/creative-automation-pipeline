...
**Demo video:** [2-minute walkthrough: install from a fresh clone, run, review](https://youtu.be/sJ_tfEEHzy0)
...
`cap demo` uses the built-in **mock provider**. It is deterministic and needs no network or key. For the demo briefs it replays real images: the heroes and outpaints that gpt-image-2 produced (through OpenRouter) when these briefs were run for real, recorded by `scripts/record_mock_fixtures.py`, so the offline run looks like the real thing. Anything it has no recording for (a new product, an edited prompt) gets a stylized placeholder, watermarked `MOCK RENDER` so nobody mistakes it for real creative. That way every reviewer can run the full pipeline, and the tests never depend on a paid API.
...
brand/tidewell/    fictional brand kit: brand.yaml, logos, Poppins + IBM Plex Sans Arabic/Hebrew (OFL)
...
scripts/           sample-logo generator, and the recorder that turns a real run into mock fixtures
```
...
```bash
pip install -e ".[dev]"
pytest          # 81 tests, ~4 min; covers schema, legal, saliency, reframing, rendering,
...
- **Scripts:** Latin scripts use Poppins. Arabic and Hebrew work: copy is right-aligned, the logo moves to the right corner, Arabic letters are joined and mixed Latin/digit runs are ordered correctly, all in pure Python (no libraqm needed). The brand kit picks the font per language under `fonts.by_language`; a language with no covering font fails the `layout.font_coverage` check instead of shipping empty boxes. CJK and other scripts need a font added the same way.
...
- The **Tidewell** brand and logos are fictional and generated by `scripts/make_sample_assets.py`. The packshot (`assets/sparkling-yuzu/hero.png`) and the images the mock replays are real gpt-image-2 generations. The fonts are Poppins and IBM Plex Sans Arabic/Hebrew (SIL OFL).
...
MIT for code. Poppins and IBM Plex Sans are licensed under the SIL Open Font License (`brand/tidewell/fonts/OFL*.txt`).
